...
### From: 49. Group Anagrams — Review (2026-07-03)
...
Mistake I made: 時間複雜度誤答 O(n log k)，忘記每次 sort 本身耗費 O(k log k)，全體應為 O(n·k log k)。

進階：改用 26-element 字母頻率 tuple 當 key，時間從 O(n·k log k) 降至 O(n·k)：
```python
count = [0] * 26
for c in word:
    count[ord(c) - ord('a')] += 1
...
### From: 1. Two Sum — Review (2026-07-15)

Input: `nums = [2, 7, 11, 15]`, `target = 9`
Approach: 用 dict 記錄「已遍歷的數字 → 索引」，每步計算 `diff = target - num`，若 diff 在 dict 中即回傳兩索引。
Key insight: 「先查後存」保證不重用同一 index——當前元素在查找時還未入 dict，因此不可能自配對。
